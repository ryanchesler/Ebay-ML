...
x = tf.placeholder(tf.float32, shape=[None, 352])
y_ = tf.placeholder(tf.float32, shape=[None, 2])
W = tf.Variable(tf.zeros([352,2]))
...
print(accuracy.eval(feed_dict={x: batch_xs, y_: batch_ys}))
...
x_image = tf.reshape(x, [-1,32,20,1])
...
W_fc1 = weight_variable([4 * 5 * 128, 1024])
b_fc1 = bias_variable([1024])

h_pool2_flat = tf.reshape(h_pool2, [-1, 4*5*128])
